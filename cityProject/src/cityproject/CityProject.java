...
public class CityProject {
    ...
    public static void main(String[] args) {
        ...
        shortPath(cityCount, cities, linkCount, links);
...
// method to create an adjacency lists for each city
    ...
    public static void shortPath(int cityCount, City[] cities, int linkCount, Edge[] e) {

        int i = 0; // loop conter
        boolean visited = false;

        boolean U = visited;
        int bestDistance = Integer.MAX_VALUE;   //maximum value and integer can have

        AdjacencyNode currentVertex = cities[i].getAdjacencyListHead();

        currentVertex.setDistance(0);

        currentVertex.getCity().setImmediatePredecessor(null);

        // declare variable that get the city name from the keyboard
        String sour = cities[i].getAdjacencyListHead().getCity().getName(); // hold the siurce city

        String dest = cities[i].getAdjacencyListHead().getNext().getCity().getName(); // hold the destination cities

        // declare en instane of scanner to read the data stream from the keyboard
        Scanner keyboard = new Scanner(System.in);

        // get the city name
        System.out.println(" Hello! please inter the city nam: ");
        sour = keyboard.nextLine();

        // get the city name
        System.out.println(" Hello! please inter the city name: ");
        ...
        while (U = false) {
            ...
            for (i = 0; i < cityCount; i++) {
                ...
                if (currentVertex.getNext().getcDistance() < currentVertex.getCity().getBestDistance()) {

                    bestDistance = currentVertex.getCity().getBestDistance(); // hold the current best Distance

                    currentVertex.getCity().setImmediatePredecessor(currentVertex.getCity()); // hold the current immediate predecessor

                    currentVertex.getCity().getVisited();  // current city get visisted

                    currentVertex.getCity().equals(U); // cuurent cite is placed in U

                    // relaxation
                    bestDistance = currentVertex.getcDistance() + currentVertex.getCity().getBestDistance();  // hold the bestDistance apdated
